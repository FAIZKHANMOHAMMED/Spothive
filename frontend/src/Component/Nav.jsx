...
import logo from '../assets/stayspot-logo.png'
...
function Nav() {
    ...
    const handleFilterChange = (filters) => {
        ...
        filtered = filtered.filter(item =>
            item.rent >= filters.priceRange[0] && item.rent <= filters.priceRange[1]
        );

        // Filter by guest count
        if (filters.guestCount > 1) {
            filtered = filtered.filter(item => item.guests >= filters.guestCount);
        }
        ...
        if (filters.bathrooms > 0) {
            filtered = filtered.filter(item => item.bathrooms >= filters.bathrooms);
        }
        ...
        if (filters.sortBy) {
            switch (filters.sortBy) {
                ...
                case 'price-high':
                    ...
                    break;
                case 'rating':
                    filtered.sort((a, b) => (b.averageRating || b.ratings) - (a.averageRating || a.ratings));
                    break;
                case 'newest':
        ...
        <div className='fixed top-0 bg-[white] z-[20]'>
            <div className='w-[100vw] min-h-[80px]  border-b-[1px] border-[#dcdcdc] px-[20px] flex items-center justify-between md:px-[40px] '>
                ...
                <div className='flex items-center justify-center    gap-[10px] relative'>
                    <button
                        className='text-[18px] cursor-pointer rounded-[50px] hover:bg-[#ded9d9] px-[12px] py-[8px] hidden md:flex items-center gap-2 border border-gray-300'
